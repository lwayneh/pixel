...
func NewTexture(width, height int, smooth bool, pixels []uint8) *Texture {
	...
	gl.TexParameterfv(gl.TEXTURE_2D, gl.TEXTURE_BORDER_COLOR_EXT, &borderColor[0])
...
func (t *Texture) Pixels(x, y, w, h int) []uint8 {
	...
	textureObj := t.tex.obj
	var fbo uint32

	gl.GenFramebuffers(1, &fbo)
	gl.BindFramebuffer(gl.FRAMEBUFFER, fbo)
	gl.FramebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textureObj, 0)
	gl.ReadPixels(int32(x), int32(y), int32(w), int32(h), gl.RGBA, gl.UNSIGNED_BYTE, gl.Ptr(pixels))
